...
class QuasiStableColoringGraph:
    def __init__(self, colored_graph: ColoredGraph, q=0.0, n_colors=np.inf, weighting=False, q_tolerance=0.0, logger=None, logging_level="error"):
    ...
    def refine(self):
        ...
        while len(self.partitions) < self.n_colors:
            ...
            q_error = np.max(errors)

            if q_error > q_error_before:
                self.logger.warning(f"[iteration {iteration}] Q-ERROR JUMPED: from {q_error_before:.1f} to {q_error:.1f}")
                self.logger.warning("--------------------")

            threshold = q_error * (1 - self.q_tolerance)
            witness_pairs = np.argwhere(errors >= threshold)

            self.logger.info(
                f"[iteration {iteration}] Found {len(witness_pairs)} witness pairs with q-error >= {threshold:.1f} "
                f"(max={q_error:.1f}, tolerance={self.q_tolerance:.2f})"
            )
